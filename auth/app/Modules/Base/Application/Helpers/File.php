...
function uploadImage($name, $title, $folder, $disk = 'public')
{
    if (is_string($name) && isBase64($name)) {
        // Convert base64 to UploadedFile
        $name = convertBase64ToUploadedFile($name);
    }
    if (!$name) return null;
    $path = $name->store("uploads/{$folder}", [
        'disk' => $disk
    ]);
...
{
    return preg_match('/^data:(image\/(jpeg|png|gif|webp|svg\+xml)|video\/(mp4|webm|ogg|avi)|audio\/(mpeg|wav|ogg)|application\/(pdf|msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document));base64,/', $string);
}
