...
class EmployeeApiService implements AuthenticatesViaToken
{
    ...
    public function __construct()
    {
        ...
        $this->connectUrl = env('API_CONNECT');
    }
    ...
    public function fetchTeachers($teacherIds)
    {
        try {
            ...
            $response = Http::accept('application/json')
                ...
                ->post($this->connectUrl . 'fetch_employees', $payload);
    ...
    {
        try {
            $url = rtrim($this->baseUrl, '/') . '/check_authentication';
            $response = Http::accept('application/json')
                ->contentType('application/json')
                ->withHeaders([
    ...
    public function checkEmployeeTeacherAuth($token)
    {
        try {
            $response = Http::accept('application/json')
                ->contentType('application/json')
